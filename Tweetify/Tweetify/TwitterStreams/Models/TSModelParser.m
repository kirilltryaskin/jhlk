...
+ (void)parseJson:(id)json
      ...
      deleteTweet:(TSModelParserTweet)deleteTweet
       ...
       userupdate:(TSModelParserUnsupported)userupdate
    ...
    dispatch_async(operation_processing_queue(), ^(void) {
        if ([json isKindOfClass:[NSDictionary class]]) {
            ...
            if ([json objectForKey:@"friends"]) {
                ...
                model.deleteTwID = [[[json objectForKey:@"delete"]objectForKey:@"status"]objectForKey:@"id_str"];
            ...
            else if ([json objectForKey:@"event"] && [[json objectForKey:@"event"] isEqualToString:@"user_update"]) {
                TSUser* model = [[[TSUser alloc] initWithDictionary:json] autorelease];
